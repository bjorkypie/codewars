...
function plant(seed, water, fert, temp){
  //declare number of plant parts from parameters
  let flower = seed;
  let stem = '-'
  let stemLen = stem.repeat(water);
  let numFlowers = flower.repeat(fert)
  //determin whether normal temp range
  let normal = temp <= 30 && temp >= 20 ? true : false;
  //return the proper stem and flower pattern based on normal temperature
  return normal ? (stemLen + numFlowers).repeat(water) : stemLen.repeat(water) + flower;
